...
internal sealed class DataGridBridge : IDataGridComponent, IDisposable
{
    ...
    {
        var internalResult = await _importManager.ImportFromDictionaryAsync(data, checkboxStates, startRow, insertMode, timeout, progress, cancellationToken);
        // Convert internal ImportResult to public ImportResult
        return new ImportResult(
            RowsProcessed: internalResult.ImportedRows,
            ErrorCount: internalResult.ErrorRows,
            Errors: internalResult.Errors.ToArray()
        );
    }
    ...
    public async Task<ImportResult> ImportFromDataTableAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var internalResult = await _importManager.ImportFromDataTableAsync(dataTable, checkboxStates, startRow, insertMode, timeout, progress, cancellationToken);
        // Convert internal ImportResult to public ImportResult
        return new ImportResult(
            RowsProcessed: internalResult.ImportedRows,
            ErrorCount: internalResult.ErrorRows,
    ...
    public async Task<ImportResult> ImportFromExcelAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var internalResult = await _importManager.ImportFromExcelAsync(excelData, worksheetName, hasHeaders, checkboxStates, startRow, insertMode, timeout, progress, cancellationToken);
        // Convert internal ImportResult to public ImportResult
        return new ImportResult(
            RowsProcessed: internalResult.ImportedRows,
            ErrorCount: internalResult.ErrorRows,
    ...
    public async Task<ImportResult> ImportFromCsvAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var internalResult = await _importManager.ImportFromCsvAsync(csvContent, delimiter, hasHeaders, checkboxStates, startRow, insertMode, timeout, progress, cancellationToken);
        return new ImportResult(
            RowsProcessed: internalResult.ImportedRows,
            ErrorCount: internalResult.ErrorRows,
            Errors: internalResult.Errors.ToArray()
    ...
    public async Task<ImportResult> ImportFromJsonAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var internalResult = await _importManager.ImportFromJsonAsync(jsonContent, checkboxStates, startRow, insertMode, timeout, progress, cancellationToken);
        return new ImportResult(
            RowsProcessed: internalResult.ImportedRows,
            ErrorCount: internalResult.ErrorRows,
            Errors: internalResult.Errors.ToArray()
    ...
    public async Task<ImportResult> ImportFromXmlAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var internalResult = await _importManager.ImportFromXmlAsync(xmlContent, rootElementName, checkboxStates, startRow, insertMode, timeout, progress, cancellationToken);
        return new ImportResult(
            RowsProcessed: internalResult.ImportedRows,
            ErrorCount: internalResult.ErrorRows,
            Errors: internalResult.Errors.ToArray()
